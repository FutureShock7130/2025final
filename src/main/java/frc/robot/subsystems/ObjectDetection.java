 ...
 * This subsystem uses PhotonVision to detect objects classified as "0" (algae)
 * or "1" (coral)
...
public class ObjectDetection extends SubsystemBase {
    ...
    // Shuffleboard items
    // private final ShuffleboardTab visionTab = Shuffleboard.getTab("Object
    // Detection");
    ...
    // aimToleranceEntry = visionTab.add("Aim Tolerance (deg)",
    ...
    private double estimateDistanceFromArea(double area) {
        ...
        // Experiments show that distance is roughly proportional to the reciprocal of
    ...
    private void updateTrackedObjects(PhotonPipelineResult result) {
        ...
        for (PhotonTrackedTarget target : result.getTargets()) {
            ...
            if (objectId != 0 && objectId != 1)
                continue; // Only process objects with ID 0 or 1
    ...
    // if (newTargetClass != targetClass && (newTargetClass == 0 || newTargetClass
    // == 1)) {
    ...
    public Optional<TrackedObject> getNearestObject() {
        ...
        return trackedObjects.values().stream()
                .filter(obj -> !obj.isStale())
                .min(Comparator
                        .comparingDouble(obj -> robotPose.getTranslation().getDistance(obj.pose.getTranslation())));
    }
     ...
     * This command both aims at the target and maintains a specified distance from
     * it
     */
